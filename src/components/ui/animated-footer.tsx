...
const AnimatedFooter: React.FC<FooterProps> = ({
  ...
  copyrightText,
  barCount = 28,
  waveColor = 'rgb(220, 38, 38)', // Bliss Balance Red
}) => {
  ...
  return (
    <footer
      ref={footerRef}
      className="bg-black text-white relative flex flex-col w-full justify-between select-none overflow-hidden pt-12 border-t border-neutral-900 font-mono"
    >
      <div className="container mx-auto flex flex-col md:flex-row justify-between w-full gap-8 pb-16 pt-4 px-6 relative z-10">
        <div className="space-y-4">
          <ul className="flex flex-wrap gap-6 text-xs uppercase font-bold tracking-wider">
          ...
          <p className="text-xs text-neutral-400 mt-4 flex items-center gap-x-2 font-mono">
            <svg className="size-3.5 text-red-600 shrink-0" viewBox="0 0 80 80">
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                fill="currentColor"
        ...
        <div className="space-y-4">
          ...
          <div className="text-left md:text-right mt-4">
            <button
              onClick={scrollToTop}
              className="text-xs font-mono font-bold uppercase tracking-wider text-red-500 hover:text-white inline-flex items-center gap-1 transition-colors"
            >
              Back to top ↑
            </button>
          </div>
      ...
      {/* Red Animated Wave Canvas Bar Container */}
      <div
        id="waveContainer"
        aria-hidden="true"
        style={{ overflow: 'hidden', height: 180 }}
