...
export const EditorialCategoryGrid: React.FC = () => {
  return (
    <section className="py-16 sm:py-28 bg-[#0A0A0A] border-b border-neutral-800 relative select-none">
        ...
        <ScrollReveal direction="up">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div className="space-y-1">
              <span className="text-[11px] font-mono font-bold tracking-[0.25em] text-neutral-500 uppercase">
                Curated Silhouettes
              </span>
            ...
            <Link href="/collections" className="inline-flex items-center gap-1.5 text-xs font-mono font-bold uppercase tracking-wider text-neutral-400 hover:text-white transition-colors">
              Full Lineup <ArrowUpRight className="w-4 h-4" />
            </Link>
          </div>
        </ScrollReveal>

        {/* 2×2 Grid — NO rounded corners */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-neutral-800">
          {CATEGORY_TILES.map((tile, index) => (
            <ScrollReveal key={tile.id} direction="up" delay={index * 0.07}>
              <Link
                href={tile.link}
                className="group relative block aspect-[3/2] bg-neutral-950 overflow-hidden"
              >
                {/* Image */}
                <img
                  src={tile.image}
                  alt={tile.title}
                  loading="lazy"
                  className="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-700 ease-out brightness-70 group-hover:brightness-90"
                />
                ...
                {/* Bottom */}
                <div className="absolute inset-x-6 bottom-6 z-10 flex items-end justify-between gap-4 text-white">
                  <div>
                    <h3 className="font-heading text-xl sm:text-3xl font-black uppercase tracking-tight leading-none group-hover:text-[#E5FF00] transition-colors duration-300">
                      {tile.title}
                    </h3>
                    <p className="text-xs text-neutral-400 font-medium mt-1">{tile.subtitle}</p>
                  </div>
                  <div className="w-9 h-9 bg-white/10 border border-white/20 group-hover:bg-[#E5FF00] group-hover:border-transparent flex items-center justify-center transition-all duration-300 group-hover:scale-110 shrink-0">
                    <ArrowUpRight className="w-4 h-4 text-white group-hover:text-black stroke-[2.5]" />
                  </div>
                </div>
              </Link>
            </ScrollReveal>
