...
export const LookbookSection: React.FC = () => {
  const [activeHotspotId, setActiveHotspotId] = useState<string | null>('look-1');
  ...
  return (
    <section className="py-16 sm:py-28 bg-white dark:bg-[#0A0A0A] border-b border-neutral-200/60 dark:border-neutral-800/60 relative select-none">
      <div className="max-w-[1400px] mx-auto px-5 sm:px-8 space-y-8 sm:space-y-12">
        ...
        <ScrollReveal direction="up">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            <p className="text-xs sm:text-sm text-neutral-500 max-w-md font-medium">
              Click the hotspots to inspect silhouettes worn in real-life street campaigns.
            </p>
          </div>
        </ScrollReveal>

        {/* 4-Column Editorial Lookbook Grid */}
          ...
          {LOOKBOOK_ITEMS.map((item, index) => {
            ...
            return (
              <ScrollReveal key={item.id} direction="up" delay={index * 0.08}>
                <div 
                  className="group relative bg-neutral-900 aspect-[2/3] overflow-hidden"
                ...
                >
                  ...
                  >
                    ...
                    <button
                      ...
                      className="relative -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-black text-white hover:bg-[#E5FF00] hover:text-black border-2 border-white flex items-center justify-center shadow-2xl transition-transform hover:scale-110 focus:outline-none z-10 cursor-pointer"
                    >
                      {/* Pulse aura */}
                      <span className="absolute inset-0 border-2 border-white/60 animate-ping pointer-events-none" />
                      {isPopupOpen ? (
                    ...
                    {isPopupOpen && (
                      <div
                        className={`absolute z-30 w-52 bg-black/95 text-white p-3 border-2 border-white shadow-2xl backdrop-blur-md animate-fade-in-up ${
                          isNearRightEdge
                  ...
                  {/* Bottom Tag Bar */}
                  <div className="absolute inset-x-0 bottom-0 z-10 flex items-center justify-between p-3.5 bg-black/85 backdrop-blur-md border-t border-white/10 text-white">
                    <div className="space-y-0.5 min-w-0 pr-2">
                      <span className="text-[9px] font-mono uppercase text-neutral-400 block tracking-widest truncate">
                        {item.title}
                      </span>
                      <p className="font-heading font-bold text-xs uppercase truncate">
                        {item.productName}
                      </p>
                    </div>
                    <Link
                      href={item.productUrl}
                      className="p-2 bg-white text-black hover:bg-[#E5FF00] hover:text-black transition-colors shrink-0 border border-black"
